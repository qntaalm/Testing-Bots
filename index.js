...
const LOG_CHANNEL_ID = '1279109247266984007'; // استبدلها بمعرف قناة اللوق
const ROLE_HIERARCHY = ['1279694287436517450', '1279694365840904273', '1279694404470313031', '1279694444899340288']; // استبدلها بمعرفات الرتب بالتدرج الصحيح

client.on('messageCreate', async (message) => {
    if (message.content.startsWith(prefix + 'ترقية')) {
        const args = message.content.split(' ');
        const memberId = args[1].replace(/[<@!>]/g, '');
        const member = message.guild.members.cache.get(memberId);

        if (!member) {
            message.reply('العضو غير موجود.');
            return;
        }

        const embed = new MessageEmbed()
            .setColor('BLUE')
            .setTitle('ترقية وتخفيض')
            .setDescription(`اختر الإجراء المطلوب لهذا العضو: ${member}`);

        const row = new MessageActionRow().addComponents(
            new MessageButton().setCustomId('promote').setLabel('ترقية').setStyle('PRIMARY'),
            new MessageButton().setCustomId('demote').setLabel('تخفيض').setStyle('SECONDARY'),
            new MessageButton().setCustomId('double_promote').setLabel('دبل ترقية').setStyle('SUCCESS'),
            new MessageButton().setCustomId('cancel').setLabel('إلغاء').setStyle('DANGER')
        );
...
client.on('interactionCreate', async (interaction) => {
    ...
    const memberId = interaction.message.embeds[0].description.match(/\d+/)[0];
    const member = interaction.guild.members.cache.get(memberId);

    const currentRole = ROLE_HIERARCHY.find(roleId => member.roles.cache.has(roleId));
    const currentIndex = ROLE_HIERARCHY.indexOf(currentRole);
if (interaction.customId === 'promote' && currentIndex < ROLE_HIERARCHY.length - 1) {
        const newRole = ROLE_HIERARCHY[currentIndex + 1];
        await member.roles.add(newRole);
        await member.roles.remove(currentRole);
        logAction(interaction, member, 'ترقية', newRole);

        await interaction.update({
            embeds: [new MessageEmbed().setColor('GREEN').setTitle('تمت الترقية').setDescription(`تم ترقية ${member} إلى الرتبة الجديدة.`)],
            components: []
        });
    }

    if (interaction.customId === 'demote' && currentIndex > 0) {
        const newRole = ROLE_HIERARCHY[currentIndex - 1];
        await member.roles.add(newRole);
        await member.roles.remove(currentRole);
        logAction(interaction, member, 'تخفيض', newRole);

        await interaction.update({
            embeds: [new MessageEmbed().setColor('ORANGE').setTitle('تم التخفيض').setDescription(`تم تخفيض ${member} إلى الرتبة السفلى.`)],
            components: []
        });
    }

    if (interaction.customId === 'double_promote' && currentIndex < ROLE_HIERARCHY.length - 2) {
        const newRole = ROLE_HIERARCHY[currentIndex + 2];
        await member.roles.add(newRole);
        await member.roles.remove(currentRole);
        logAction(interaction, member, 'دبل ترقية', newRole);

        await interaction.update({
            embeds: [new MessageEmbed().setColor('GREEN').setTitle('تمت دبل الترقية').setDescription(`تم دبل ترقية ${member} إلى الرتبة الأعلى.`)],
            components: []
        });
    }
    ...
    if (interaction.customId === 'cancel') {
        await interaction.update({
            embeds: [new MessageEmbed().setColor('RED').setTitle('إلغاء العملية').setDescription('تم إلغاء العملية.')],
            components: []
        });
        logAction(interaction, member, 'إلغاء', null);
    }
});

async function logAction(interaction, member, action, newRole) {
    const logChannel = interaction.guild.channels.cache.get(LOG_CHANNEL_ID);
    const embed = new MessageEmbed()
        .setColor('BLUE')
        .setTitle('سجل العملية')
